                    ...
                    @for($i=0; $i<sizeof($invoice); $i++)
                        <tr class="card my-2" onclick="getDetails('{!! $invoice[$i]->periodo_cobranca_inicio !!}','{!! $invoice[$i]->periodo_cobranca_fim !!}', {!! $invoice[$i]->cobrado !!})">
                            <td class="card-header d-flex justify-content-between" >
                                <abbr title="HyperText Markup Language" class="initialism_alt">{!! 'Periodo de cobrança: '.\Helpers\Helpers::formatDate($invoice[$i]->periodo_cobranca_inicio).' - '.\Helpers\Helpers::formatDate($invoice[$i]->periodo_cobranca_fim); !!}</abbr>
                            </td>
                ...
                <div class="container-xxl mt-4 mobile ">
                    <div class="row">
                        <table class="table_list">
                            @for($i=0; $i<sizeof($invoice); $i++)
                                <tr class="card my-2" 
                                            onclick="getDetails('{!! $invoice[$i]->periodo_cobranca_inicio !!}','{!! $invoice[$i]->periodo_cobranca_fim !!}', {!! $invoice[$i]->cobrado !!})" 
                                            style=" box-shadow: horizontal-offset vertical-offset blur-radius spread-radius color inset;">
                                <td class="w-100 d-flex justify-content-between card-header py-4 px-4">
                                    <div>
                                    ...
                                    <div style="align-items: start !important">
                                        <div class="text-secondary">Valor R$</div>
                                        <abbr title="HyperText Markup Language" class="initialism">{!! 'R$ '.number_format($invoice[$i]->cobrado,2,",",".");; !!}</abbr>
                                    </div>
                                    <div style="align-items: start !important">
                                        <div class="text-secondary">Início do Perído</div>
                                        <abbr title="HyperText Markup Language" class="initialism">{!!  "  ".\Helpers\Helpers::formatDate($invoice[$i]->periodo_cobranca_inicio); !!}</abbr>
                                    </div>
                                    <div style="float: right !important">
                                        <div class="text-secondary">Término do Periodo</div>
                                        <abbr title="HyperText Markup Language" class="initialism">{!! "  ".\Helpers\Helpers::formatDate($invoice[$i]->periodo_cobranca_fim); !!}</abbr>
                                    </div>
